...
class Board {
    ...
    PieceColor getWinner() {
        ...
        if (!checkBoard) {
            int numPieceRed = _numPieces[2];
            int numPieceBlue = _numPieces[3];
            if (numPieceRed > numPieceBlue) {
                _winner = RED;
            } else if (numPieceBlue > numPieceRed) {
                _winner = BLUE;
            } else {
                _winner = EMPTY;
            }
    ...
    boolean legalMove(Move move) {
        ...
        PieceColor fromIndex = get(move.fromIndex());
        PieceColor toIndex = get(move.toIndex());

        if (fromIndex == BLOCKED || toIndex == BLOCKED) {
            return false;
        }

        if (!(fromIndex == whoseMove()) || toIndex != EMPTY) {
            return false;
        }
        ...
        if (move.isJump() || move.isExtend() || move.isPass()) {
            return true;
        }
        ...
        for (char col : COLS) {
            for (char row : ROWS) {
                if (get(col, row) == who) {
                    for (int i = -2; i <= 2; i++) {
                        for (int j = -2; j <= 2; j++) {
                            if (get(neighbor(index(col, row), i, j)) == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        PieceColor currColor = whoseMove();
        int fromIndex = move.fromIndex();
        int toIndex = move.toIndex();
        PieceColor fromIndexColor = get(fromIndex);

        if (move.isJump()) {
            _numJumps++;
            set(toIndex, fromIndexColor);
            set(fromIndex, EMPTY);
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    if (opponent == get(neighbor(toIndex, i, j))) {
                        set(neighbor(toIndex, i, j), fromIndexColor);
                        if (currColor == RED) {
                            _numPieces[2]++;
                            _numPieces[3]--;
                        } else {
    ...
    /** Undo the last move. */
    void undo() {
        Move move = _allMoves.get(_allMoves.size() - 1);
        int i = _undoSquares.size() - 1;
        int j = _undoPieces.size() - 1;
        if (move.isExtend()) {
            while (_undoSquares.get(i) != null) {
                if (_undoPieces.size() > 1) {
                    while (_undoPieces.get(j - 1) == whoseMove().opposite()) {
                        unrecordedSet(_undoSquares.get(i), whoseMove());
                        _numPieces[whoseMove().ordinal()]++;
                        _numPieces[whoseMove().opposite().ordinal()]--;
                        _undoPieces.remove(j); _undoSquares.remove(i);
                        i--; j--;
                    }
                    if (_undoPieces.get(j - 1) != whoseMove().opposite()) {
                        unrecordedSet(_undoSquares.get(i), EMPTY);
                        _numPieces[whoseMove().opposite().ordinal()]--;
                        _undoPieces.remove(j); _undoSquares.remove(i);
                        i--; j--;
                    }
            ...
            _numJumps--;
            while (_undoSquares.get(i) != null) {
                while (_undoPieces.get(j) == whoseMove()) {
                    unrecordedSet(_undoSquares.get(i), whoseMove());
                    _numPieces[whoseMove().ordinal()]++;
                    _numPieces[whoseMove().opposite().ordinal()]--;
                    _undoPieces.remove(j); _undoSquares.remove(i);
                ...
                if (_undoPieces.get(j) == whoseMove().opposite()) {
                    unrecordedSet(_undoSquares.get(i), whoseMove().opposite());
                    _undoPieces.remove(j);
                    _undoSquares.remove(i);
                    i--;
                    j--;
                }
                if (_undoPieces.get(j) == EMPTY) {
                    unrecordedSet(_undoSquares.get(i), EMPTY);
                    _undoPieces.remove(j);
                    _undoSquares.remove(i);
                    i--;
    ...
    void setBlock(char c, char r) {
        ...
        int index = Character.getNumericValue(r);
        int absIndex = abs(index - 4);
        int originalChar = new String(COLS).indexOf(c);
        int targetChar = new String(COLS).indexOf('d');
        int targetIndex = abs(targetChar - originalChar);
        if (c == 'd' && r == '4') {
            unrecordedSet(c, r, BLOCKED);
            _totalOpen--;
        } else if (c == 'd') {
            unrecordedSet(c, (char) (4 + absIndex + '0'), BLOCKED);
            unrecordedSet(c, (char) (4 - absIndex + '0'), BLOCKED);
            _totalOpen -= 2;
        } else if (r == '4') {
            unrecordedSet((char) ('d' + targetIndex), r, BLOCKED);
            unrecordedSet((char) ('d' - targetIndex), r, BLOCKED);
            _totalOpen -= 2;
        } else {
            char rowRight = (char) ('d' + targetIndex);
            char rowLeft = (char) ('d' - targetIndex);
            unrecordedSet(rowRight, (char) (4 + absIndex + '0'), BLOCKED);
            unrecordedSet(rowLeft, (char) (4 + absIndex + '0'), BLOCKED);
            unrecordedSet(rowRight, (char) (4 - absIndex + '0'), BLOCKED);
            unrecordedSet(rowLeft, (char) (4 - absIndex + '0'), BLOCKED);
            _totalOpen -= 4;
        }
    ...
    private int _numRedMoves;

    private int _numBlueMoves;
