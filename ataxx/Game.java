...
class Game {
    ...
    public static PlayFabSettings playFabSettings;
    ...
    int play() {
        ...
        redPlayer = myObj.nextLine();

        System.out.println("Enter Blue player name: ");
        bluePlayer = myObj.nextLine();
        //LoginToPlayfab(redPlayer);
        _board.clear();
        setManual(RED);
        setAuto(BLUE);
        _exit = -1;
        ...
        while (_exit < 0) {
            ...
            if (_board.getWinner() == null) {
                ...
                if (!winnerAnnounced) {
                    ...
                    if (_board.getWinner() == RED) {
                        UpdateStatistic(redPlayer, _board.numRedMoves());
                    } else {
                        UpdateStatistic(bluePlayer, _board.numBlueMoves());
                    }
    ...
    private void  UpdateStatistic(String playerName, int score)
    {
        LoginToPlayfab(playerName);
    ...
    private void LoginToPlayfab(String redPlayer) {
        PlayFabSettings.TitleId = "C2B11";
        PlayFabSettings.DeveloperSecretKey = "HI63O3EUOY8RB99X7Q447CZ5358S91UQZNQS5RQ3IJBYQM3XXQ";
        PlayFabClientModels.LoginWithCustomIDRequest request = new PlayFabClientModels.LoginWithCustomIDRequest();
        request.TitleId = "C2B11";
        request.CustomId = redPlayer;

        request.CreateAccount = true;

        System.out.println("Login in to playfab....");

        PlayFabErrors.PlayFabResult<PlayFabClientModels.LoginResult> result = PlayFabClientAPI.LoginWithCustomID(request);
   ...
   /** Used to return a move entered from the console.  Allocated
    ...
    private final Leaderboard _leader = new Leaderboard();
}
